...
internal static class FundingDomainEventCollector
{
    public static IReadOnlyCollection<FundingDomainEvent> Collect(ChangeTracker changeTracker)
    {
        ...
        foreach (var entry in changeTracker.Entries<QualificationFundings>())
        {
            AddFundingChange(
                events,
                entry,
                RolloverSourceTypes.Ofqual,
                nameof(QualificationFundings.QualificationVersionId),
                nameof(QualificationFundings.FundingOfferId),
                nameof(QualificationFundings.StartDate),
        ...
        where TEntity : class
    {
        ...
        var sourceQualificationId = (Guid)entry.Property(sourceIdProperty).CurrentValue!;
        var fundingOfferId = (Guid)entry.Property(fundingOfferIdProperty).CurrentValue!;
        Guid? previousSourceQualificationId = sourceMoved
            ? (Guid)entry.Property(sourceIdProperty).OriginalValue!
            : null;
