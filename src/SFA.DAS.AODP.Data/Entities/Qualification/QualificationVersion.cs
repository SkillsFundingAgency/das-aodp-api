
namespace SFA.DAS.AODP.Data.Entities.Qualification;

public class QualificationVersion
{
    public Guid Id { get; set; }
    public Guid QualificationId { get; set; }
    public Guid VersionFieldChangesId { get; set; }
    public Guid ProcessStatusId { get; set; }
    public int AdditionalKeyChangesReceivedFlag { get; set; }
    public Guid LifecycleStageId { get; set; }
    public string OutcomeJustificationNotes { get; set; } = string.Empty;
    public Guid AwardingOrganisationId { get; set; }
    public string Status { get; set; } = string.Empty;
    public string Type { get; set; } = string.Empty;
    public string Ssa { get; set; } = string.Empty;
    public string Level { get; set; } = string.Empty;
    public string SubLevel { get; set; } = string.Empty;
    public string EqfLevel { get; set; } = string.Empty;
    public string GradingType { get; set; } = string.Empty;
    public string GradingScale { get; set; } = string.Empty;
    public int TotalCredits { get; set; }
    public int Tqt { get; set; }
    public int Glh { get; set; }
    public int MinimumGlh { get; set; }
    public int MaximumGlh { get; set; }
    public DateTime RegulationStartDate { get; set; }
    public DateTime OperationalStartDate { get; set; }
    public DateTime OperationalEndDate { get; set; }
    public DateTime CertificationEndDate { get; set; }
    public DateTime ReviewDate { get; set; }
    public bool OfferedInEngland { get; set; }
    public bool OfferedInNi { get; set; }
    public bool OfferedInternationally { get; set; }
    public string Specialism { get; set; } = string.Empty;
    public string Pathways { get; set; } = string.Empty;
    public string AssessmentMethods { get; set; } = string.Empty;
    public string ApprovedForDelFundedProgramme { get; set; } = string.Empty;
    public string LinkToSpecification { get; set; } = string.Empty;
    public string ApprenticeshipStandardReferenceNumber { get; set; } = string.Empty;
    public string ApprenticeshipStandardTitle { get; set; } = string.Empty;
    public bool RegulatedByNorthernIreland { get; set; }
    public string NiDiscountCode { get; set; } = string.Empty;
    public string GceSizeEquivelence { get; set; } = string.Empty;
    public string GcseSizeEquivelence { get; set; } = string.Empty;
    public string EntitlementFrameworkDesign { get; set; } = string.Empty;
    public DateTime LastUpdatedDate { get; set; }
    public DateTime UiLastUpdatedDate { get; set; }
    public DateTime InsertedDate { get; set; }
    public int Version { get; set; }
    public bool AppearsOnPublicRegister { get; set; }
    public int LevelId { get; set; }
    public int TypeId { get; set; }
    public int SsaId { get; set; }
    public int GradingTypeId { get; set; }
    public int GradingScaleId { get; set; }
    public bool PreSixteen { get; set; }
    public bool SixteenToEighteen { get; set; }
    public bool EighteenPlus { get; set; }
    public bool NineteenPlus { get; set; }
    public string ImportStatus { get; set; } = string.Empty;
}
